/**
 * French orthophoto sub-régions (NUTS-1). All share one IGN Géoplateforme
 * BD ORTHO® data source, so they are pure `name → départements` data —
 * the pipeline logic lives in `fr_shared.ts`.
 */

import type { RegionPipeline } from '../lib/framework.ts';
import { defineFrSubRegion } from './fr_shared.ts';
