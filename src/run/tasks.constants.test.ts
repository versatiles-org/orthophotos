...
test('TASKS - contains all 5 task definitions', () => {
	expect(TASKS.length).toBe(5);
});

test('TASKS - task numbers are 0-4', () => {
...
test('TASK_NAME_TO_NUMBER - maps names correctly', () => {
	...
	expect(TASK_NAME_TO_NUMBER['upload']).toBe(3);
	expect(TASK_NAME_TO_NUMBER['delete']).toBe(4);
});
...
test('TASK_NAME_TO_NUMBER - includes aliases', () => {
	...
	expect(TASK_NAME_TO_NUMBER['3_upload']).toBe(3);
	expect(TASK_NAME_TO_NUMBER['4_delete']).toBe(4);
});
...
test('TASK_NUMBER_TO_NAME - provides inverse mapping', () => {
	...
	expect(TASK_NUMBER_TO_NAME[3]).toBe('upload');
	expect(TASK_NUMBER_TO_NAME[4]).toBe('delete');
});
...
test('VALID_TASK_NUMBERS - contains all valid numbers', () => {
	...
	expect(VALID_TASK_NUMBERS.has(5)).toBe(false);
	expect(VALID_TASK_NUMBERS.has(-1)).toBe(false);
});

test('ALL_TASKS - has correct pipeline order', () => {
	// Pipeline order: 0 1 3 2 3 4
	expect(ALL_TASKS).toEqual([0, 1, 3, 2, 3, 4]);
});
